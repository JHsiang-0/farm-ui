...
- 实现内容：确认用户 enabled/disabled 字段、批量失败项重试语义、统一分页和错误码；记录前后端基线版本。
...
- 优先级：P2
- 前置依赖：T109
- 修改文件：待新增日志 API、页面/组件和测试
- 实现内容：在后端提供正式审计日志契约后展示用户、动作、目标、时间和结果；当前不伪造数据。
- 验收标准：无后端接口时显示“规划中”且不可操作；有接口后支持权限和分页。
- 测试方式：契约测试、权限和分页测试。
- 需要后端配合：是，需要新增正式 API。
- 需要真实打印机：否。
- 风险：当前交接文档无日志接口。
- 完成状态：BLOCKED_BACKEND。当前交接文档未提供正式操作日志 API、字段、权限和分页契约，保持规划中且不可操作；待后端契约冻结后再实现。
...
第一批可立即开始的任务是 T001–T006，但实际代码改造应在 T001 的两个契约问题得到确认后，从 T002/T005 开始。
