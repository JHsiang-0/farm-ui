...
- [ ] 以 TDesign Starter 基础仪表盘为视觉基线统一侧栏、顶栏、面包屑、页面标题、内容留白、卡片、筛选区、数据区和操作区。
- [ ] 建立 FabMatrix 主题 Token，只通过 TDesign CSS Token 和项目语义 Token 管理主色、背景、文字、边框、成功、警告、危险、阴影、圆角和间距；不得在业务页散落无语义颜色。
- [ ] 默认使用浅色主题；保留可扩展暗色主题的 Token 结构，但不得在未完成全页面适配时提供半成品开关。
- [ ] 侧边导航按“工作台、打印机、文件、任务、批量派发、管理中心、个人中心”分组，折叠、激活、权限过滤、窄屏抽屉和 Electron 窗口场景行为一致。
- [ ] 统一页面容器、筛选工具栏、数据卡片、详情抽屉、表单弹窗、危险确认、状态标签、分页、空态和错误态的复用组件及样式入口。
- [ ] 清理旧品牌文案和工程展示名，统一用户可见品牌为 `FabMatrix`；不得破坏登录页现有布局和动画。

#### T208-2 登录、初始化、会话与权限界面

- [ ] 保留 `Login.vue` 的双栏构图、动画角色、输入布局、移动端布局和主要动效，不套用 Starter 后台壳，不改造成通用后台登录模板。
- [ ] 在保留视觉的前提下统一 TDesign 的 loading、message、表单错误、密码可见、键盘操作和可访问性反馈。
- [ ] 覆盖首次管理员初始化、普通登录、登录失败、初始化状态读取失败、账号禁用、会话过期、并发 401、退出登录和刷新恢复场景。
- [ ] 为 403、404 和无法识别的角色提供明确结果页或稳定的页面内反馈；OPERATOR 不能看到或直接访问 ADMIN 菜单和操作。
- [ ] 不新增公开注册页；管理员创建 OPERATOR 继续放在用户管理内部界面。
